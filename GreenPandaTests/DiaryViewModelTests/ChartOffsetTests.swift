...
class ChartOffsetTests: XCTestCase {
    ...
    var capturedChartOffset: Double!
    ...
    override func setUp() {
        ...
        diaryViewModel = ModelBackedDiaryViewModel(model: mockGreenPandaModel,
                                        ...
                                        coordinatorDelegate: StubDiaryViewModelCoordinatorDelegate())
        diaryViewModel.entriesPublisher.sink(receiveValue: {
            self.capturedEntries = $0
        }).store(in: &bag)

        diaryViewModel.chartOffsetPublisher.sink(receiveValue: {
            self.capturedChartOffset = $0
        }).store(in: &bag)
    ...
    func testThatCorrectXPositionIsReturnedFromViewModel() {
        ...
        XCTAssertEqual(capturedChartOffset, Double(date2033Jun22_08_08_35))
    }
    ...
    func testThatChartXOffsetIsSetWhenUserScrolls() {
        ...
        XCTAssertEqual(capturedChartOffset, Double(1602444114.6 - 7*24*60*60))
    }
    ...
    func testThatCorrectChartXOffsetIsSetWhenUserScrollsToTheLastEntry() {
        ...
        XCTAssertEqual(capturedChartOffset, Double(date2020Sep20_23_51_51))
    }
...
private class StubDiaryViewModelCoordinatorDelegate : DiaryViewModelCoordinatorDelegate {
